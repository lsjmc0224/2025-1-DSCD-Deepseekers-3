...
class AnalysisService:
    ...
    def run_batch_analysis(self):
        ...
        log_id = self.repo.create_analysis_log(started_at=started_at)
        ...
        for item in unanalyzed_items:
            ...
            for result in analysis_results:
                all_results.append(ContentAnalysis(
                    ...
                    evidence_keywords=", ".join(result["evidence_keywords"])
                ))

        # 3. 분석 결과 저장
        if all_results:
            self.repo.create_content_analysis_results(all_results)

        # 4. 원본 is_analyzed 업데이트
        self.repo.mark_as_analyzed([item.get_original() for item in unanalyzed_items])

        # 5. 로그 종료 시간 기록
        self.repo.finish_analysis_log(log_id=log_id, finished_at=datetime.now())
